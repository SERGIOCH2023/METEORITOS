...
onready var camaraplayer:CamaraPlayer=$Player/CamaraPlayer
...
func crear_sector_meteoritos(centro_camara:Vector2, numero_peligros:int)-> void:
	...
	camara_nivel.zoom = camaraplayer.zoom
	camara_nivel.devolver_zoom_original()
	camaraplayer.devolver_zoom_original()
	transicion_camaras(
...
func controlar_meteoritos_restantes()-> void:
	...
	if meteoritos_totales == 0:
		...
		camaraplayer.set_puede_hacer_zoom(true)
		var zoom_actual = camaraplayer.zoom
		camaraplayer.zoom = camara_nivel.zoom
		camaraplayer.zoom_suavizado(zoom_actual.x, zoom_actual.y,1.0)
		transicion_camaras(
			camara_nivel.global_position,
			camaraplayer.global_position,
			camaraplayer,
